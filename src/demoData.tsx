...
export const columns: TableColumnsType<Member> = [
  ...
  { title: '팀', dataIndex: 'team', key: 'team', width: 120, filters: ['Design', 'Platform', 'Growth', 'Data', 'Product', 'Mobile'].map((value) => ({ text: value, value })), onFilter: (value, record) => record.team === value, filterSearch: true },
  { title: '상태', dataIndex: 'status', key: 'status', width: 100, filters: ['활성', '휴가', '대기'].map((value) => ({ text: value, value })), onFilter: (value, record) => record.status === value, render: (value) => <span className={`status status--${value}`}>{String(value)}</span> },
...
export const largeData: Member[] = Array.from({ length: 1000 }, (_, index) => ({ key: `V-${index + 1}`, name: `구성원 ${String(index + 1).padStart(4, '0')}`, role: index % 2 ? 'Frontend Engineer' : 'Product Designer', team: ['Design', 'Platform', 'Growth'][index % 3], status: index % 7 === 0 ? '휴가' : '활성', projects: index % 15, joinedAt: `202${index % 5}-0${(index % 9) + 1}-12` }))
