...
| \`trigger\` | hover, focus, click 중 표시 동작을 하나 이상 설정해요. | \`TooltipTrigger \\| TooltipTrigger[]\` | \`hover\` |
  ...
  render: (args) => (
    <div className="flex min-h-32 flex-wrap items-center justify-center gap-3">
      ...
      <Tooltip {...args} color="#0062df" title="파란색">
        <Button type="secondary">색상</Button>
      </Tooltip>
      <Tooltip {...args} arrow={false} title="화살표 없음">
        <Button type="secondary">화살표 없음</Button>
      </Tooltip>
    </div>
  ),
};
...
export const Controlled: Story = {
  ...
  return (
    <Tooltip open={open} title="Tooltip" onOpenChange={setOpen}>
      <Button type="secondary" onClick={() => setOpen((current) => !current)}>
        {open ? '닫기' : '열기'}
      </Button>
    </Tooltip>
  );
