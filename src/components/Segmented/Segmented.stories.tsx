...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "Segmented는 같은 범주의 선택지를 나란히 놓고 하나를 빠르게 전환해요.  \n크기·방향·아이콘과 선택 상태를 지정할 수 있어요.",
      },
      page: () => (
...
Segmented는 관련된 선택지를 한 줄에 나열하고 하나를 선택하게 해요.

| Name | Description | Type | Default |
| --- | --- | --- | --- |
| \`options\` | 선택 항목을 설정해요. | [\`SegmentedItem[]\`](#segmenteditem) | - |
| \`value\` | 선택 값을 외부에서 관리해요. | \`Key\` | - |
| \`defaultValue\` | 처음 선택할 값을 설정해요. | \`Key\` | 첫 활성 항목 |
...
| \`onChange\` | 선택 값이 바뀔 때 실행해요. | \`(value: Key) => void\` | - |

### SegmentedItem

SegmentedItem은 하나의 선택지에 표시할 값과 레이블을 정의해요.

| Name | Description | Type | Default |
| --- | --- | --- | --- |
| \`value\` | 항목의 고유 값을 설정해요. | \`Key\` | - |
...
| \`tooltip\` | 항목에 Tooltip을 연결해요. | \`string\` | - |
      `}</Markdown>
          ...
          <p>SegmentedSizeType은 Segmented 항목의 높이와 글자 크기를 구분해요.</p>
...
export const Controlled: Story = {
  ...
  const [value, setValue] = useState<Key>('week');
  const options = [
    { value: 'day', label: '일간' },
    { value: 'week', label: '주간' },
    { value: 'month', label: '월간' },
