  ...
  args: {
    rowSelection: {
      getCheckboxProps: (record) => ({
        ...
        title: record.status === "대기" ? "대기 구성원은 선택할 수 없습니다." : undefined,
      }),
    },
  },
};
export const FixedSelectionColumn: Story = {
  args: { rowSelection: { fixed: true }, scroll: { x: 1600 } },
};
export const CustomSelectionCell: Story = {
  args: {
    rowSelection: {
      renderCell: (checked, record, _index, node) => (
        <span title={`${record.name}: ${checked ? "선택됨" : "선택 안 됨"}`}>{node}</span>
      ),
    },
  },
};
export const SelectionMenu: Story = { args: { rowSelection: { selections: true } } };
export const SelectionConstants: Story = {
  args: {
    rowSelection: {
...
export const ShiftRangeSelection: Story = {
  args: { rowSelection: {} },
  parameters: {
    docs: {
      description: {
        story: "첫 체크박스를 선택한 뒤 Shift를 누른 채 다른 행을 선택하면 범위가 선택됩니다.",
      },
    },
  },
};
export const ControlledSelection: Story = {
  render: (args) => <ControlledSelectionStory {...args} />,
};
  ...
  return (
    <>
      <div className="mb-4 flex gap-2">
        <button
          type="button"
          className="h-8 rounded border border-[#ddd] px-3 text-[#111] hover:bg-[#f5f5f5] disabled:opacity-40"
...
export const SelectionColumnConfiguration: Story = {
  args: {
    rowSelection: {
      align: "right",
      columnWidth: 72,
      columnTitle: (checkbox) => (
        <span className="inline-flex items-center gap-1.5 whitespace-nowrap">선택 {checkbox}</span>
      ),
      getTitleCheckboxProps: () => ({ title: "현재 페이지 전체 선택" }),
      onCell: (record) => ({ title: `${record.name} 선택 셀` }),
    },
  },
};

export const CustomSelectionAction: Story = {
  render: (args) => <CustomSelectionActionStory {...args} />,
};

function CustomSelectionActionStory(args: TableProps<Member>) {
  const [selectedRowKeys, setSelectedRowKeys] = useState<Key[]>([]);
  const activeKeys = members
    .filter((member) => member.status === "활성")
    .map((member) => member.key);
  return (
    <>
      <p className="mb-4 text-[#999]">
        선택 메뉴의 "활성 구성원 선택"으로 사용자 정의 일괄 작업을 실행합니다.
      </p>
      <Table<Member>
        {...args}
        rowSelection={{
          ...
          selections: [
            {
              ...
              onSelect: (changeableKeys) =>
                setSelectedRowKeys(
                  changeableKeys.filter((key) => activeKeys.includes(String(key))),
                ),
            },
          ],
        }}
...
export const PreserveSelectionAcrossDataChanges: Story = {
  render: (args) => <PreserveSelectionStory {...args} />,
};
    ...
    <>
      <div className="mb-4 flex items-center gap-3">
        <button
          ...
          onClick={() => setPage((current) => (current ? 0 : 1))}
        >
          데이터 교체 (현재 {page + 1})
        </button>
        <span className="text-[#999]">보존된 선택 {selected.length}개</span>
      </div>
      <Table<Member>
        {...args}
        dataSource={pageData}
        rowSelection={{ preserveSelectedRowKeys: true, onChange: (keys) => setSelected(keys) }}
      />
    </>
  );
}

export const HiddenSelectAllAndDefaults: Story = {
  args: { rowSelection: { hideSelectAll: true, defaultSelectedRowKeys: ["M-1001", "M-1002"] } },
};

export const SelectionCallbacks: Story = {
  render: (args) => <SelectionCallbacksStory {...args} />,
};
  ...
  return (
    <>
      ...
      <Table<Member>
        {...args}
        rowSelection={{
          onChange: (keys, _rows, info) => setMessage(`onChange: ${info.type} / ${keys.length}개`),
          onSelect: (record, selected) =>
            setMessage(`onSelect: ${record.name} ${selected ? "선택" : "해제"}`),
        }}
      />
    </>
  );
