...
export const SemanticClassNamesAndStyles: Story = {
  args: { columns, classNames: { header: { cell: 'text-[#0062df]' }, body: { row: 'even:[&>td]:bg-[#fafafa]' } }, styles: { cell: { fontVariantNumeric: 'tabular-nums' } } },
};
...
export const SharedColumnDefaults: Story = { args: { columns, column: { align: 'center', className: 'text-[#0062df]' } } };

export const RowAndHeaderHooks: Story = {
  args: {
    columns,
    rowClassName: (record) => (record.status === '휴가' ? '[&>td]:bg-[#fffbe6]' : ''),
    onRow: (record) => ({ title: `${record.name} 데이터 행`, 'aria-label': `${record.name} 데이터 행` }),
...
export const CustomTableComponents: Story = { args: { columns, components: { body: { row: StoryBodyRow } }, rowClassName: (_record, index) => (index % 2 ? '[&>td]:bg-[#fafafa]' : '') } };
  ...
  return (
    <>
      <div className="mb-4 flex gap-2">
        <button type="button" className="h-8 rounded border border-[#ddd] px-3 text-[#111] hover:bg-[#f5f5f5]" onClick={() => ref.current?.scrollTo({ index: 0, align: 'start' })}>
          첫 행
        </button>
        <button type="button" className="h-8 rounded border border-[#ddd] px-3 text-[#111] hover:bg-[#f5f5f5]" onClick={() => ref.current?.scrollTo({ key: 'V-75', align: 'center' })}>
          75번째 행
        </button>
        <button type="button" className="h-8 rounded border border-[#ddd] px-3 text-[#111] hover:bg-[#f5f5f5]" onClick={() => ref.current?.scrollTo({ index: 99, align: 'end' })}>
          마지막 행
        </button>
      </div>
      <Table<Member> {...args} ref={ref} dataSource={largeData.slice(0, 100)} columns={columns} virtual scroll={{ x: 900, y: 300 }} />
    </>
  );
}
  ...
  args: {
    columns: columns.map((column) =>
      column.key === 'role'
        ? { ...column, hidden: true }
        : column.key === 'name'
          ? { ...column, ellipsis: { showTitle: false }, minWidth: 180, onHeaderCell: () => ({ className: 'bg-[#e6f4ff]' }) }
          : column.key === 'team'
            ? { ...column, onCell: (record: Member) => ({ title: `${record.team} 팀`, className: record.team === 'Design' ? 'bg-[#f6ffed] font-semibold text-[#237804]' : undefined }) }
            : column,
    ),
  },
};
