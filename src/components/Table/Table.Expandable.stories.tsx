...
export const ExpandByRowClick: Story = {
  args: { expandable: { expandRowByClick: true, expandedRowRender: (record) => <div>{record.name} 상세 정보</div> } },
};
  ...
  args: {
    expandable: {
      ...
      expandIcon: ({ expanded, expandable, record, onExpand }) => (
        ...
        >
          <svg viewBox="0 0 12 12" aria-hidden className={`size-2.5 stroke-current transition-transform ${expanded ? 'rotate-90' : ''}`} fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}>
...
export const HideExpandColumn: Story = {
  args: { expandable: { showExpandColumn: false, expandRowByClick: true, expandedRowRender: (record) => <div>{record.name} 상세 정보</div> } },
};
...
const customChildrenData: NestedMember[] = [{ ...members[0], nodes: [{ ...members[5], key: 'M-1001-node', name: '한지우 (nodes 하위)' }] }, ...members.slice(1, 5)];
    ...
    <>
      ...
      <Table<Member>
        ...
        expandable={{
          ...
          onExpand: (expanded, record) => setMessage(`onExpand: ${record.name} ${expanded ? '펼침' : '접힘'}`),
